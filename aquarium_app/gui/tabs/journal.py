...
from aquarium_app.config import (
    ...
    COLOR_OK_BG, COLOR_ALT_ROW, COLOR_WARN, COLOR_ACCENT_SOFT, COLOR_ACCENT_HOVER,
...
class JournalTab:
    ...
    def _build_journal_day_card(self, day_data, volume_l, aq_id, targets, today_iso):
        ...
        # левый столбец — показания (всегда отображается)
        ...
        if has_readings:
            ...
            tk.Label(left_col, text="— показаний не было —", bg=COLOR_BG,
                     fg=COLOR_TEXT_MUTED, font=(FF, 9)).pack(anchor="w", padx=6, pady=2)

        # правый столбец — удобрения (всегда отображается)
